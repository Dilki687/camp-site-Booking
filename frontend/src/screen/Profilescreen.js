...
export function MyBookings({ bookingDetails }) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (isReservation) {
            // Cancel reservation
            const reservationResponse = await axios.post("/api/reservations/cancelbooking", {
              bookingid,
            });
            console.log("Reservation cancellation response:", reservationResponse.data);
          } else {
            // Cancel booking from the booking model
            const bookingResponse = await axios.post("/api/bookings/cancelbooking", {
              bookingid,
              camplocationid,
            });
  ...
  const generateReport = () => {
    ...
    // Load the logo from the local path and add it to the PDF
    const logoPath = `${process.env.PUBLIC_URL}/images/logo.jpeg`; // Ensure the path is accessible
    const img = new Image();
    img.src = logoPath;
  
    // Center the logo and resize it to 1x1 inch (72x72 points)
    img.onload = () => {
      const imgWidth = 36; // 1/2 inch width
      const imgHeight = 36; // 1/2 inch height
      const centerX = (pageWidth - imgWidth) / 2;
      doc.addImage(img, "JPEG", centerX, 10, imgWidth, imgHeight);
  
      // Calculate the center position for the "Booking Report" title below the logo
      const title = "Booking Report";
      const textWidth = doc.getTextWidth(title);
      const centerTitleX = (pageWidth - textWidth) / 2;
      const titleY = 60; // Adjust this value to set the vertical position below the logo
  
      doc.setFontSize(20);
      doc.text(title, centerTitleX, titleY); // Center the title below the logo
  
      // Add rest of the report after the title
      ...
      doc.setFontSize(12);
      doc.text(`Downloaded by: ${user ? user.name : "Dilki"}`, 10, 90);
      doc.text(`Downloaded on: ${dateString} at ${timeString}`, 10, 100);
      ...
      doc.autoTable({
        ...
        body: tableData,
        startY: 110, // Start after the title
        columnStyles: {
          0: { cellWidth: 25 },
          1: { cellWidth: 25 },
          2: { cellWidth: 25 },
